...
public class HomeAccounting
{
    ...
    public static void Save(string fileName, List<ExpensesIncomes> e)
    {
        StreamWriter file = new StreamWriter(fileName);
        for (int i = 0; i < e.Count; i++)
        {
            file.WriteLine(e[i].date);
            file.WriteLine(e[i].description);
            file.WriteLine(e[i].category);
            file.WriteLine(e[i].amount);
        }
        file.Close();
    }
    ...
    public static List<ExpensesIncomes> Load(string fileName)
    {
        ...
        do
        {
            ...
            {
                newExp.date = date;
                newExp.description = file.ReadLine();
                newExp.category = file.ReadLine();
                newExp.amount = Convert.ToSingle(file.ReadLine());
                e.Add(newExp);
            }
        } while (date != null);
