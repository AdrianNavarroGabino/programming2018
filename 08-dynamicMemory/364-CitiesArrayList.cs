            ...
            switch (option)
            {
                ...
                case 2: // View all the cities
                    for (int i = 0; i < countCities; i++)
                    {
                        ...
                        Console.WriteLine("Name: {0}", ((City) c[i]).name);
                ...
                case 3: // Edit a city
                    ...
                    Console.Write("Enter name of city (it was {0}): ",
                        ((City) c[cityToEdit]).name);
                    string newName = Console.ReadLine();
                    if (newName != "")
                        editCity.name = newName;
                    else
                    {
                        editCity.name = ((City) c[cityToEdit]).name;
                    }

                    Console.Write("Enter population of city (it was {0}): ",
                        ((City) c[cityToEdit]).population);
                    ...
                    {
                        editCity.population = ((City) c[cityToEdit]).population;
                    }
                ...
                case 4: // Search in cities
                    ...
                    for (int i = 0; i < countCities; i++)
                    {
                        if (((City) c[i]).name.ToUpper().Contains(search))
                        {
                            Console.WriteLine("City {0}:", i + 1);
                            Console.WriteLine("Name: {0}", ((City) c[i]).name);
                            Console.WriteLine("Population: {0}",
